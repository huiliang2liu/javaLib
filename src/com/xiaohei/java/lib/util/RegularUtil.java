...
/**
 * com.xh.utils author:liuhuiliang email:825378291@qq.com instruction:
...
public class RegularUtil {
    ...
    public static int year(String card) {
        if (!isIDCard(card))
            return -1;
        String year = card.length() == 15 ? getIdcardCalendar()
                + card.substring(6, 8) : card.substring(6, 10);

        return Integer.parseInt(year);
    }

    public static int month(String card) {
        if (!isIDCard(card))
            return -1;
        String month = null;
        if (card.length() == 15) {
    ...
    public static int day(String card) {
        if (!isIDCard(card))
            return -1;
        String day = null;
        if (card.length() == 15) {
    ...
    public static String birthDath(String card) {
        if (!isIDCard(card))
            return null;
        if (card.length() == 15) {
            return getIdcardCalendar() + card.substring(6, 12);
        } else {
    ...
    public static String sex(String card) {
        String sex = "";
        if (isIDCard(card) || card.length() == 18) {
            sex = Integer.parseInt(card.substring(16, 17)) % 2 == 0 ? "女" : "男";
        }
        return sex;
    }

    public static String provinces(String card) {
        String sex = "";
        if (isIDCard(card) || card.length() == 18) {
            sex = zoneNum.get(Integer.parseInt(card.substring(0, 2)));
        }
        return sex;
    }
