package com.xiaohei.java.lib.reflect;
...
public class FieldManager extends XHReflect {
    ...
    public static boolean setField(Object object, Field field, Object value) {
        ...
        try {
            ...
            if (Modifier.isStatic(modifiers) && Modifier.isFinal(modifiers)) {//注意编译时被处理，所以编译时能确定的量不能被替换
                Field modi=field.getClass().getDeclaredField("modifiers");
                modi.setAccessible(true);
                modi.setInt(field, modifiers & ~Modifier.FINAL);
            }
